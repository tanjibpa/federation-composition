...
describe("FederatedLink", () => {
  ...
  test("resolveImportName", () => {
    ...
    const federationLink = links.find(
    ...
    // unimported
    expect(federationLink?.resolveImportName("@external")).toBe(
    ...
    // default import
    const linkLink = links.find(
      (l) => l.identity === "https://specs.apollo.dev/link",
    );
    expect(linkLink?.resolveImportName("@link")).toBe("link");

    // unnamespaced
    const unnamedLink = links.find(
      (l) => l.identity === "https://unnamed.graphql-hive.com",
    );
    ...
    // imported as
    const hiveLink = links.find(
      (l) => l.identity === "https://specs.graphql-hive.com/hive",
    );
